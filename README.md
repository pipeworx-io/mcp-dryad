...
curl -X POST https://gateway.pipeworx.io/v1/tools/dryad_search_datasets \
  ...
  -d '{"query":"coral bleaching"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/dryad_search_datasets`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
